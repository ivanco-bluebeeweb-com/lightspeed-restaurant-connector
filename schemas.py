"""Pydantic schemas for Lightspeed Restaurant Connector."""
...
class ConnectParams(BaseModel):
    label: str = Field(default="", description="Friendly connection label, e.g. Primary Lightspeed Restaurant.")
    access_token: str = Field(description="POS Access Token / API Key")
    base_url: str = Field(default="https://api.lightspeed.app/restaurant/v1", description="Lightspeed Restaurant API base URL.")
...
class GetOrderParams(BaseModel):
    connection_id: str = Field(default="", description="Optional connection ID.")
    order_id: str = Field(description="Lightspeed Restaurant Order ID.")
