"""Connection lifecycle for Lightspeed Restaurant Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from lightspeed_restaurant_client import LightspeedRestaurantClient
...
_SECRET = "lightspeed_restaurant_connections"
...
@chat.function(
    "connect_lightspeed_restaurant",
    "Connect Lightspeed Restaurant account via credentials.",
    action_type="write",
    chain_callable=True,
    event="lightspeed-restaurant-connector.connect_lightspeed_restaurant",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_lightspeed_restaurant(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Lightspeed Restaurant Connector."""
    client = LightspeedRestaurantClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Lightspeed Restaurant Account",
...
@chat.function(
    "list_connections",
    "List connected Lightspeed Restaurant accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_lightspeed_restaurant",
    "Disconnect Lightspeed Restaurant account.",
    action_type="write",
    chain_callable=True,
    event="lightspeed-restaurant-connector.disconnect_lightspeed_restaurant",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_lightspeed_restaurant(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
